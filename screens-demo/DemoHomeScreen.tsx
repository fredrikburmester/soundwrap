...
export default function DemoHomeScreen({ navigation }: RootStackScreenProps<'DemoHome'>) {
  ...
  const cards = [
    {
      ...
      onPress: () => navigation.navigate('DemoTopSongs'),
    ...
    },
    {
      ...
      onPress: () => navigation.navigate('DemoTopArtists'),
      icon: 'person-outline'
    },
  ]
  ...
  useEffect(() => {
    navigation.setOptions({
      ...
      headerRight: () => (
        <TouchableOpacity onPress={() => navigation.navigate('DemoProfile')}>
  ...
  return (
    <ScrollView style={{ paddingHorizontal: 18, backgroundColor: Colors.background, paddingTop: 18 }} contentInsetAdjustmentBehavior="automatic">
      <Card title="Play the game" style={{ backgroundColor: Colors.primary, shadowColor: Colors.primaryLight, shadowRadius: 5, shadowOpacity: 1, shadowOffset: { width: 0, height: 1 } }} description="Play the game!" onPress={() => navigation.navigate('DemoSoundcheck')} />
